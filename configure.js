...
function getIAMUsername() {
  callerIdentityArn = child
    .execSync("aws sts get-caller-identity --query Arn --output text")
    .toString()
    .replace(/\n/g, "");
...
async function applyTemplates() {
  ...
  let { projectSpoken } = await prompts({
    ...
    validate: (value) =>
      value.match(/[^a-z'. ]/i)
        ? "Invocation name may only contain alphabetic characters, apostrophes, period, and spaces"
        : true,
    initial: spokenSuggestion,
  });
  projectSpoken = projectSpoken.toLowerCase();
  const author = `${authorName}` + authorEmail ? `<${authorEmail}>` : "";
...
async function configureJovoProjectFiles() {
  ...
  if (shouldInitializeJovo) {
    child.execSync(`node setup_jovo.js`, { stdio: "inherit" });
  } else {
  ...
  if (shouldInitialize) {
    child.execSync(`node setup_terraform.js`, { stdio: "inherit" });
  } else {
    console.log(
      `No worries. Just run ./setup_terraform.js when you're ready to deploy to staging or production.`
    );
  }
}

async function configureCODEOWNERS() {
  const prompts = require("prompts");
  const { ownCode, name, email } = await prompts([
    {
      type: "select",
      name: "ownCode",
      message:
        "Would you like to be assigned as the CODEOWNER for this project?",
      choices: [
        { title: "No", value: false },
        { title: "Yes", value: true },
      ],
    ...
    {
      type: (previous) => (previous ? "text" : null),
      message: "What is your github @username or email?",
      name: "email",
      initial: authorEmail.includes("volley") ? authorEmail : "",
    },
  ]);

  let codeownerContent =
    "# You can add your github @username or email to this file to be notified of PRs in this repository\n";
  if (ownCode) {
    codeownerContent += `${email} \t# ${name}`;
    codeownerContent += "\n";
  }
  fs.writeFileSync(".github/CODEOWNERS", codeownerContent);
}
